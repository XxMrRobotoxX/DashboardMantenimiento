...
try:
    ...
    if (date_filter == ()):
        ...
        df_filtered = data[data["Maquina"].isin(maquinas)]
        df_filtered = df_filtered[(df_filtered["Estatus"] == "Cerrada") & (df_filtered["CausoParo"] == "Si") & (df_filtered['FechaInicio'].between(date_filter[0].strftime('%d/%m/%Y'),date_filter[1].strftime('%d/%m/%Y'), inclusive='both'))]
