...
def load_data(url):
    ...
    df['Start_DT'] = pd.to_datetime(df['FechaInicio'] + ' ' + df['HoraInicio'])
    df['End_DT'] = pd.to_datetime(df['FechaFin'] + ' ' + df['HoraFin'])
...
try:
    ...
    df_filtered = df_filtered[df_filtered["Estatus"] == "Cerrada"]
